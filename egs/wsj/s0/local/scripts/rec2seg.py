...
parser = argparse.ArgumentParser(description="Split long recordings into segments of recordings. (NOTE: We normalize the segments into the 16-bit range, less accuracy but more friendly to hear than sox segmentation).\n\nLong recordings are at the `wav.scp' file, each line with format of <recording_id> <recording_path>\nSegments of recordings are at the `segments' file, each line with format of <utterance_id> <recording_id> <begin_sec> <end_sec>.\nOutput short segmented recordings named as <utterance_id>.wav\nSee more details about 16-bit normalization at https://simpleaudio.readthedocs.io/en/latest/tutorial.html.\nThe 16-bit normalization might make low amplitude sound into normal volume\n(make it cover the whole amplitude rage but not exceeding it), more friendly for hearing.", formatter_class=argparse.RawTextHelpFormatter)
