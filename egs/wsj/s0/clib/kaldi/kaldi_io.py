...
# wget https://raw.githubusercontent.com/vesis84/kaldi-io-for-python/master/kaldi_io.py
# format and add docs by bin-wu
...
def read_key(fd):
    ...
    assert(re.match(r'^\S+$', key) != None)  # check format (no whitespace!)
    return key
...
def read_vec_int(file_or_fd):
    ...
    if binary == '\0B':  # binary flag
        ...
        vec = np.frombuffer(
            fd.read(vec_size*5), dtype=[('size', 'int8'), ('value', 'int32')], count=vec_size)
...
def _read_compressed_mat(fd, format):
    ...
    col_headers = np.array([np.array([x for x in y]) * globrange *
                            1.52590218966964e-05 + globmin for y in col_headers], dtype=np.float32)
    ...
    mat += (p25 + (p75 - p25) / 128. * (data - 64)) * \
        mask_65_192.astype(np.float32)
    mat += (p75 + (p100 - p75) / 63. * (data - 192)) * \
        mask_193_255.astype(np.float32)
