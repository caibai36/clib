...
#    -> v4: Replace CNN with ViT
#       v4: Updated to ViT ver1 with a linear warmup and cosine annealing scheduler at 18:13 on 12 November 2024
#    -> v5: use pretrained MAE ViT
...
# MAE ViT specific arguments
parser.add_argument("--model_type", type=str, default="mae", choices=["vit", "mae"],
                    help="Model type to use (vit or mae)")
parser.add_argument("--mae_pretrained", type=str, default="base",
...
parser.add_argument("--shared_encoder", action="store_true",
                    help="Use a shared encoder for both streams (copy weights from the first to the second)")
...
parser.add_argument("--patch_size", type=int, default=16, help="Size of patch")
parser.add_argument("--num_classes", type=int, default=9, help="Number of classes")
parser.add_argument("--dim", type=int, default=768, help="Dimension of ViT")
parser.add_argument("--depth", type=int, default=12, help="Depth of ViT")
...
class MAEViTEncoder(nn.Module):
    ...
    Args:
        ...
        mae_pretrained (str): Path to pretrained MAE model or 'base' for facebook/vit-mae-base. Default: 'base'
        mae_train_layers (int): Number of layers to train when using pretrained model. Default: -1 (all layers)
    """
...
class TwoStreamMAEViTModel(nn.Module):
    """
    A two-stream Vision Transformer model using MAE encoders.

    This model consists of two parallel MAE-ViT encoders processing two input streams,
    followed by shared fully connected layers for classification.

    Args:
        mae_params (dict): Parameters for the MAE-ViT encoder.
        dropout_rate (float): The dropout rate for the shared layers.
        shared_dim (int): The dimension of the shared fully connected layer.
        shared_encoder (bool): Whether to use a shared encoder for both streams (default: False).
            If True, the weights of the first encoder will be copied to the second.
    """

    def __init__(self, mae_params, dropout_rate, shared_dim, shared_encoder=False):
        super(TwoStreamMAEViTModel, self).__init__()
        self.dropout_rate = dropout_rate
        self.shared_dim = shared_dim

        # First MAE-ViT stream
        self.mae1 = MAEViTEncoder(**mae_params)

        # Second MAE-ViT stream (optionally shared with the first)
        if shared_encoder:
            # Create a new encoder with the same parameters
            self.mae2 = MAEViTEncoder(**mae_params)
            # Copy weights from the first encoder
            self.mae2.load_state_dict(self.mae1.state_dict())
        else:
            # Independent second encoder
            self.mae2 = MAEViTEncoder(**mae_params)

        # Shared fully connected layers
        vit_output_dim = mae_params['dim']  # Get dimension from MAE parameters
        self.fc1 = nn.Linear(vit_output_dim * 2, self.shared_dim)
        self.dropout = nn.Dropout(p=self.dropout_rate)
        self.fc2 = nn.Linear(self.shared_dim, 9)
        self.fc3 = nn.Linear(self.shared_dim, 9)
    ...
    def forward(self, x1, x2, y1=None, y2=None, mode='train'):
        ...
        Returns:
            ...
            If y1 and y2 are not provided:
                tuple: A tuple containing:
                    ...
                    - attention_weights (list): A list of attention weight tensors.
        """
        # Get latent representation from first MAE-ViT encoder
        x1, attn1 = self.mae1(x1)

        # Get latent representation from second MAE-ViT encoder
        x2, attn2 = self.mae2(x2)

        # Concatenate the latent representations from both encoders
        x = torch.cat((x1, x2), dim=1)
        ...
        # Generate outputs for both streams
        logits1 = self.fc2(x)
        logits2 = self.fc3(x)
        ...
        # Return predictions and attention weights for inference
        return probs1, probs2, attention_weights
...
def setup_optimizer_and_scheduler(model, base_lr, batch_size, weight_decay, warmup_epochs, total_epochs, constant_base_lr=True):
    ...
    This function initializes an AdamW optimizer with a learning rate that can be
    either constant or scaled based on the batch size. It also creates a learning
    rate scheduler that combines a linear warmup phase with a cosine annealing phase.
    ...
    Args:
        ...
        total_epochs (int): Total number of epochs including a linear warmup and a cosine annealing phases.
        constant_base_lr (bool, optional): If True, use the base_lr as is; if False, scale the learning rate based on batch size. Defaults to True.

    Returns:
        tuple:
            - optimizer (torch.optim.Optimizer): The initialized AdamW optimizer.
            - scheduler (torch.optim.lr_scheduler._LRScheduler): The combined
              linear warmup and cosine annealing learning rate scheduler.

    Note:
        If constant_base_lr is False, the actual learning rate is calculated as:
        actual_lr = base_lr * (batch_size / 256)
        If constant_base_lr is True, the actual learning rate is simply base_lr.
    """
    # Determine the actual learning rate
    if constant_base_lr:
        actual_lr = base_lr
    else:
        actual_lr = base_lr * (batch_size / 256) # Setting from MAE and DINO
...
def train(model, optimizer, scheduler, train_loader, dev_loader, num_epochs, eval_epoch_interval, model_path):
    """
    ...
    while epoch < num_epochs:
        ...
        # Step the learning rate scheduler and log the current learning rate
        scheduler.step()
...
if __name__ == '__main__':
    ...
    if args.model_type == "vit":
        ...
        logger.info(f"Initialized ViT model")
    else:  # model_type == "mae"
        mae_params = {
            'image_size': args.image_size,
            'patch_size': patch_size,
        ...
        model = TwoStreamMAEViTModel(mae_params, dropout_rate=args.dropout_rate,
                                     shared_dim=args.shared_dim,
                                     shared_encoder=args.shared_encoder).to(device)

        logger.info(f"Initialized MAE model" +
                    (f" with pretrained weights from {args.mae_pretrained}" if args.mae_pretrained else ""))
        if args.shared_encoder:
            logger.info("Using shared encoder for both streams")

    # Set up optimizer and scheduler
    # The learning rate schedule includes:
    # - Linear warmup for the first few epochs
    # - Cosine annealing decay for the remaining epochs
    ...
    train(model, optimizer, scheduler, train_loader, dev_loader, num_epochs, eval_epoch_interval, model_path)
