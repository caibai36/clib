...
exp_dir=exp/run_mae_vit
eval_model="model_best_dev.ckpt" # Set model to evaluate: model_e20.ckpt (epoch 20), model.ckpt (latest), model_best_dev.ckpt (best dev score)
...
epsilon=0.001 # for adam optimizer
num_epochs=100 # 20
...
image_size=224 # 256 # Size of input image
patch_size=16 # Size of patch
num_classes=9 # Number of classes
dim=768 # Dimension of ViT
...
model_type=mae # Model type (vit or mae)
...
shared_encoder=false # Whether to use a shared encoder for both streams
...
# Updated parameter string to include MAE-specific parameters
vit_params="dim${dim}depth${depth}heads${heads}mlpdim${mlp_dim}patch${patch_size}dropout${vit_dropout}sharedim${shared_dim}mae${model_type}pretrained${mae_pretrained:+$(basename ${mae_pretrained} .ckpt)}trainlayers${mae_train_layers}"
...
if [ ${stage} -le 2 ]; then
    date
    echo "Train mit mae vit 72..."
    python local/mit_mae_vit_train_72_torch_v1.1.py \
       --train_input1 exp/data/$dataset_name/train_input1 \
...
if [ ${stage} -le 3 ]; then
    date
    echo "Test mit mae vit 72..."
    python local/mit_mae_vit_test_72_torch_v1.1.py \
       --test_input1 exp/data/$dataset_name/test_input1_$first \
...
if [ ${stage} -le 5 ]; then
    date
    echo "Evalute mit mae vit 72..."
    rm -f $eval_dir/results.txt

    for cutoff in $cutoffs; do
    echo Cutoff: $cutoff | tee -a $eval_dir/results.txt
    python local/mit_cnn_eval_acc.py \
