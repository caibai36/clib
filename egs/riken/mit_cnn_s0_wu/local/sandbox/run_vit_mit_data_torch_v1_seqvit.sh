...
exp_dir=exp/run_seq_vit_sch
eval_model="model.ckpt" # "model_best_dev.ckpt" # Set model to evaluate: model_e20.ckpt (epoch 20), model.ckpt (latest), model_best_dev.ckpt (best dev score)
...
num_epochs=100 # 20
eval_epoch_interval=1 # evaluate and save every x epochs
...
result_dir=$exp_dir/$dataset_name/$data_name/${model_name}-${run}/bs${batch_size}baselr${base_lr}wd${weight_decay}warmup${warmup_epochs}total${total_epochs}avgpredwin${avg_pred_win}seed${seed}_${vit_params}/train
mkdir -p $result_dir
if [ ${stage} -le 2 ]; then
    date
    echo "Train mit vit 72..."
    python local/mit_vit_train_72_torch_v1.py --train_input1 exp/data/$dataset_name/train_input1 \
       --seq \
...
eval_dir=$exp_dir/$dataset_name/$data_name/${model_name}-${run}/bs${batch_size}baselr${base_lr}wd${weight_decay}warmup${warmup_epochs}total${total_epochs}avgpredwin${avg_pred_win}seed${seed}_${vit_params}/eval
mkdir -p $eval_dir
if [ ${stage} -le 3 ]; then
    date
    echo "Test mit vit 72..."
    python local/mit_vit_test_72_torch_v1.py --test_input1 exp/data/$dataset_name/test_input1_$first \
       --seq \
