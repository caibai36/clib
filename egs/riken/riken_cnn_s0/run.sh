...
data_dir_name=mit_sample0
model_name=cnn
exp_dir=exp/sys

# Options for data
data_div_yaml="conf/data/division_sample.yaml"  # YAML file containing data division by IDs for train, dev, and test sets
label2id_yaml="conf/dict/label2id_marmoset.yaml"  # YAML file containing label-to-labelID mapping
...
test_id=Athos # One test id in $data_div_yaml file

# Options for training and evaluation
num_epochs=3
save_epoch_interval=2 # save model every x epochs
batch_size=25
lr=0.0003
...
if [ ${stage} -le 1 ]; then
    ...
    ./local/mit_sample_data_prep.sh --mit_sample "$mit_sample" --data_name ${data_dir_name} --stage 0
    date
fi
