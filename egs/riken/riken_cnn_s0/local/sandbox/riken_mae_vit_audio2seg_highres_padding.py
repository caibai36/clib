...
def create_spec_data(wav_file, segment_duration=2.5, sample_rate=48000, nfft=512, win_size=0.5, keep_last_incomplete_segment=False, use_padding=False):
    """
    Generates spectrogram data from an input WAV file with optional padding.
    ...
    for i in range(num_segs):
        ...
        hop_length = int((win_size*48000 - nfft) // (256 - 1))
...
def extract_spectrogram_segments_with_padding(audio_file, window_size=0.5, window_shift=0.15, nfft=512, sample_rate=48000, use_padding=False):
    """
    Extract spectrogram segments from an audio file using a sliding window with optional padding.
