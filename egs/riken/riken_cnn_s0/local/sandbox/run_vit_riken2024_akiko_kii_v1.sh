...
data_dir_name=riken2024_akiko_kii
model_name=vit  # Changed from cnn to vit
exp_dir=exp/sys_vit

# Options for data
data_div_yaml="conf/data/division_akiko_kii.yaml"  # YAML file containing data division by IDs for train, dev, and test sets
...
if [ ${stage} -le 3 ]; then
    ...
    python local/riken_vit_train_v1.py \
...
if [ ${stage} -le 4 ]; then
    date
    echo "Test riken vit..."
    python local/riken_vit_test_v1.py \
