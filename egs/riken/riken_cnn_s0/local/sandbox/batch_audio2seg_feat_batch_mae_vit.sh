arch="mae_vit"
model="conf/model/RIKEN2024pretrained_b0family3010_mae_base_48days_best_dev.ckpt"
...
feat_batch_size=20480 # batch size for the feature extraction; not batch size for mae vit
gpu=auto
...
for file in $(cd "$data_dir"; find . -type f -name "*.wav" | sort); do
    ...
    python local/sandbox/riken_mae_vit_audio2seg_highres_feat_batch_padding.py \
