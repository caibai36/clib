...
# v3: Add call-only metrics on actual vocalizations by excluding noise segments from consideration
...
def evaluate_metrics(signal_correct, noise_correct, prediction_list, correct_list, classes, window_size=0.5, window_shift=0.05, middle_part=0.05):
    """
    Evaluates two distinct sets of performance metrics for vocalization classification:
    1. Traditional metrics that include noise segments
    2. Call-only metrics that focus exclusively on segments containing vocalizations

    The call-only metrics provide a more focused evaluation of the model's performance
    on actual vocalizations by excluding noise segments from consideration.

    Args:
        signal_correct (list): List of booleans indicating if signal (call) predictions were correct
        noise_correct (list): List of booleans indicating if noise predictions were correct
        prediction_list (list): List of paths to prediction files
        correct_list (list): List of paths to reference files
        classes (set): Set of valid call types/classes
        window_size (float): Size of sliding window in seconds (default: 0.5)
        window_shift (float): Window shift size in seconds (default: 0.05)
        middle_part (float): Size of middle window section in seconds (default: 0.05)

    Returns:
        dict: Dictionary containing both traditional and call-only metrics:
            - Traditional metrics:
                - accuracy_noise: Accuracy for noise segments
                - accuracy_signal: Accuracy for call segments
                - accuracy_total: Overall accuracy
                - precision: Traditional precision including noise
                - recall: Traditional recall including noise
                - f_score: Traditional F1-score including noise
            - Call-only metrics:
                - precision_call: Precision for call segments only
                - recall_call: Recall for call segments only
                - f_score_call: F1-score for call segments only
    """
    def compare_segments_with_predictions(prediction_list, correct_list, classes, window_size=0.5, window_shift=0.05, middle_part=0.05):
        """
        Analyzes predictions specifically for segments containing vocalizations.

        This function focuses on segments where the reference contains a call (not noise),
        tracking both the predicted and reference labels for these segments.
        ...
        Args:
            ...
            window_shift (float): Window shift size in seconds
            middle_part (float): Size of middle window section in seconds

        Returns:
            tuple: (predictions_on_calls, reference_calls)
                - predictions_on_calls: List of predicted labels for call segments
                - reference_calls: List of reference labels for call segments
        """
        predictions_on_calls = []  # Store predicted labels when reference is a call
        reference_calls = []       # Store reference call labels

        # Process each pair of prediction and reference files
        for pred_file, corr_file in zip(prediction_list, correct_list):
            with open(pred_file, 'r') as predictions, open(corr_file, 'r') as correct:
                # Discretize segments into fixed-size windows
                lines_pred = discretize_segments(predictions, classes, window_size, window_shift, middle_part)
                lines_corr, first, last = discretize_segments(correct, classes, window_size, window_shift, middle_part, return_indices=True)

            # Extend predictions with noise if shorter than reference
            lines_pred.extend(['noise'] * (len(lines_corr) - len(lines_pred)))

            # Only collect predictions for segments where reference contains a call
            for pred_label, corr_label in zip(lines_pred[first:last], lines_corr[first:last]):
                if corr_label != 'noise':  # Reference segment contains a call
                    predictions_on_calls.append(pred_label)
                    reference_calls.append(corr_label)

        return predictions_on_calls, reference_calls
    ...
    # Calculate traditional metrics (including noise segments)
    ...
    # Calculate traditional precision, recall, and F-score
    precision = true_positives / (true_positives + false_positives) if true_positives + false_positives > 0 else 0
    recall = true_positives / (true_positives + false_negatives) if true_positives + false_negatives > 0 else 0
    f_score = 2 * precision * recall / (precision + recall) if precision + recall > 0 else 0

    # Calculate call-only metrics
    predictions_on_calls, reference_calls = compare_segments_with_predictions(
        prediction_list, correct_list, classes, window_size, window_shift, middle_part
    )

    # Convert to numpy arrays for efficient computation
    predictions_on_calls = np.array(predictions_on_calls)
    reference_calls = np.array(reference_calls)

    # Calculate metrics for call segments only
    true_positives_call = np.sum(predictions_on_calls == reference_calls)  # Correct call predictions
    false_positives_call = np.sum((predictions_on_calls != 'noise') &
                                 (predictions_on_calls != reference_calls))  # Wrong call type predictions
    false_negatives_call = np.sum(predictions_on_calls == 'noise')  # Missed calls (predicted as noise)

    # Calculate call-only precision, recall, and F-score
    precision_call = true_positives_call / (true_positives_call + false_positives_call) if (true_positives_call + false_positives_call) > 0 else 0
    recall_call = true_positives_call / (true_positives_call + false_negatives_call) if (true_positives_call + false_negatives_call) > 0 else 0
    f_score_call = 2 * precision_call * recall_call / (precision_call + recall_call) if precision_call + recall_call > 0 else 0

    # Print results
    print("\nTraditional metrics (including noise):")
    ...
    print("\nCall-only metrics (excluding noise):")
    print("Recall:{:.4f}, Precision:{:.4f}, F1-score:{:.4f}".format(
        recall_call, precision_call, f_score_call))
...
metrics = evaluate_metrics(signal_correct, noise_correct, prediction_list, correct_list,
                         classes, window_size=args.window_size,
                         window_shift=args.split_size,
                         middle_part=args.split_size)
