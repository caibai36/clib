...
out_dir="nas5/familybooth_1594F_1449M_3010" # actual out_dir: "exp/out/${arch}_${model_base}/$out_dir/$file_dir/${file_base}_model_${arch}_${model_base}.txt"b
...
for file in $(cd "$data_dir"; find . -type f -name "*.wav" | sort); do
    ...
    python local/sandbox/riken_cnn_audio2seg_highres.py \
        --eval_model "$model" \
