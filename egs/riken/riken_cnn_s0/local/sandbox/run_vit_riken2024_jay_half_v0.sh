...
model_name=vit  # Changed from cnn to vit
exp_dir=exp/sys_vit
...
vit_params="dim${dim}depth${depth}heads${heads}mlpdim${mlp_dim}patch${patch_size}dropout${dropout}"
result_dir=$exp_dir/$dataset_name/$data_name/${model_name}-${run}/bs${batch_size}lr${lr}lrdecay${lr_decay_interval}avgpredwin${avg_pred_win}_${vit_params}/train
mkdir -p $result_dir
if [ ${stage} -le 3 ]; then
    date
    echo "Train riken vit..."
    python local/riken_vit_train.py \
...
eval_dir=$exp_dir/$dataset_name/$data_name/${model_name}-${run}/bs${batch_size}lr${lr}lrdecay${lr_decay_interval}avgpredwin${avg_pred_win}_${vit_params}/eval
mkdir -p "$eval_dir"
if [ ${stage} -le 4 ]; then
    date
    echo "Test riken vit..."
    python local/riken_vit_test.py \
...
if [ ${stage} -le 5 ]; then
    date
    echo "Get segments from riken vit..."
...
if [ ${stage} -le 6 ]; then
    date
    echo "Evaluate segments from riken vit..."
