...
def extract_audio_features_batch(audio_paths, begin_secs, end_secs, batch_size=32, sample_rate=48000, cache_size=100):
    ...
    Returns:
        list: List of spectrogram features, each of shape (257, 256) in float16 format.
            The features maintain the same order as the input lists.
    """
    ...
    for i in range(0, len(audio_paths), batch_size):
        ...
        for path, begin, end in zip(batch_paths, batch_begins, batch_ends):
            ...
            if expected_length is None:
                ...
                assert signal_piece.shape[1] == expected_length, \
                    f"All signals should have same length: expected {expected_length}, got {signal_piece.shape[1]}"
        ...
        # Create spectrogram
        logger.info(f"Computing spectrograms")
        nfft = 512
        ...
        # Modify this line to remove the extra dimension
        features.extend([feat[0].astype(np.float16) for feat in batch_features])  # Add [0] to remove first dimension
...
def main():
    ...
    # Update process_dataset calls to include batch_size
