...
def process_dataset(
    ...
    output_csv: Path,
    logger: DataExtractionLogger,
    freesurfer_base2: Optional[Path] = None
) -> None:
    """
    ...
    Args:
        ...
        logger: Logger for tracking extraction patterns
        freesurfer_base2: Optional secondary base directory (e.g., for age >50 months)
    """
    ...
    for subject_id in subject_ids:
        ...
        # If not found and secondary directory provided, try there
        if not subject_dir.exists() and freesurfer_base2:
            dataset_dir2 = freesurfer_base2 / dataset_name
            subject_dir = dataset_dir2 / subject_id
...
Examples:
  ...
  # NEW: Dual directory mode for subjects across age ranges
  %(prog)s \\
    --freesurfer-dir /path/to/infant_freesurfer/age_le_50_months \\
    --freesurfer-dir2 /path/to/freesurfer/age_gt_50_months \\
    --dataset new_england \\
    --new-england-subjects conf/mri/subjects/subjects_new_england_t1_age_ge_2_scans_no_extraction_err.txt \\
    --new-england-output exp/mri/affective_vocalization_analysis_ge_2_scans/batch_outputs/new_england/etiv_new_england.csv
        """
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Optional secondary FreeSurfer directory (e.g., for age >50 months subjects)'
    )
    ...
    if args.dataset in ['calgary', 'both']:
        if not args.calgary_subjects.exists():
            ...
            process_dataset(
                ...
                args.calgary_output,
                logger,
                Path(args.freesurfer_dir2) if args.freesurfer_dir2 else None
            )
