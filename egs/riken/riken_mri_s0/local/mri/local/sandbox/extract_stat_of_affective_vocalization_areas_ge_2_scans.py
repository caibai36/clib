...
Handles both infant FreeSurfer (≤50 months) and standard FreeSurfer (>50 months)
BACKWARD COMPATIBLE with original script interface
"""
...
def detect_freesurfer_type(stats_dir):
    """
    Detect whether this is infant or standard FreeSurfer output
    ...
    Returns:
        'infant' or 'standard'
    """
    # Check for files that only exist in standard FreeSurfer
    standard_markers = [
    ...
    for marker in standard_markers:
        if os.path.exists(os.path.join(stats_dir, marker)):
            return 'standard'
...
def infer_subject_id_from_stats_dir(stats_dir):
    ...
    Returns:
        Subject ID string
    """
    # Extract subject ID from path like: .../subject_id/stats/
...
def extract_subject_data(subject_id, stats_dir, config_csv, dataset_name, verbose=False):
    ...
    Returns:
        tuple: (DataFrame with all extracted measurements, has_errors boolean)
    """
    ...
    return pd.DataFrame(results), has_errors

def process_multiple_subjects(subjects_file, subjects_dir, config_csv, output_dir, dataset_name, verbose=False, subjects_dir2=None):
    """
    Process multiple subjects from a text file
    Supports dual directory mode for age-split data

    Args:
        subjects_file: Text file with subject IDs
        subjects_dir: Primary subjects directory
        config_csv: Configuration CSV
        output_dir: Output directory
        dataset_name: Dataset name
        verbose: Verbose output
        subjects_dir2: Secondary subjects directory (optional, for age > 50 months)
    """
    ...
    for i, subject_id in enumerate(subjects, 1):
        ...
        if not os.path.exists(stats_dir) and subjects_dir2:
            # Try secondary directory if provided
            stats_dir = os.path.join(subjects_dir2, subject_id, 'stats')

        if not os.path.exists(stats_dir):
            print(f"  Warning: Stats directory not found: {stats_dir}")
            subjects_with_errors.append(subject_id)
            continue

        # Extract data
        df, has_errors = extract_subject_data(subject_id, stats_dir, config_csv, dataset_name, verbose)
        all_results.append(df)

        # Track subjects with/without errors
        if has_errors:
            subjects_with_errors.append(subject_id)
        else:
            subjects_without_errors.append(subject_id)
    ...
    # Save subject lists with and without errors
    subjects_file_basename = os.path.splitext(os.path.basename(subjects_file))[0]

    # Subjects WITH errors
    if subjects_with_errors:
        error_file = os.path.join(output_dir, f"{subjects_file_basename}_with_extraction_err.txt")
        with open(error_file, 'w') as f:
            for subject_id in subjects_with_errors:
                f.write(f"{subject_id}\n")
        print(f"\n{'='*60}")
        print(f"Subjects WITH extraction errors ({len(subjects_with_errors)}): {error_file}")
        print(f"{'='*60}")

    # Subjects WITHOUT errors
    if subjects_without_errors:
        no_error_file = os.path.join(output_dir, f"{subjects_file_basename}_no_extraction_err.txt")
        with open(no_error_file, 'w') as f:
            for subject_id in subjects_without_errors:
                f.write(f"{subject_id}\n")
        print(f"Subjects WITHOUT extraction errors ({len(subjects_without_errors)}): {no_error_file}")
        print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples:
  # BACKWARD COMPATIBLE - Original command format:
  ...
  # NEW - Dual directory mode for age-split data:
  python %(prog)s \\
    --subjects-dir /path/to/infant_freesurfer/age_le_50_months/new_england \\
    --subjects-dir2 /path/to/freesurfer/age_gt_50_months/new_england \\
    --subjects-file subjects_ge_2_scans.txt \\
    --dataset new_england \\
    --output exp/mri/outputs
        '''
    )
    ...
    parser.add_argument('--subjects-dir', type=str,
                       help='Base directory containing subject directories (primary/≤50 months)')
    parser.add_argument('--subjects-dir2', type=str,
                       help='Secondary directory for subjects >50 months (optional)')
    ...
    if args.stats_dir:
        ...
        df, has_errors = extract_subject_data(subject_id, args.stats_dir, args.config, args.dataset, args.verbose)
        ...
        process_multiple_subjects(args.subjects_file, args.subjects_dir,
                                 ...
                                 subjects_dir2=args.subjects_dir2)
