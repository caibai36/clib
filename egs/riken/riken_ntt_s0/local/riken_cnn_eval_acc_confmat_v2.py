...
# Update to v2 to support TIMIT no noise segments by bin-wu at 19:33 on 20 May 2025
...
def plot_confusion_matrix(confusion_matrix, labels, output_file):
    ...
    fig, ax = plt.subplots(figsize=(25*1.5, 20*1.5))  # Even larger for better readability
    
    # Normalize the confusion matrix for better visualization
    cm_normalized = confusion_matrix.astype('float') / confusion_matrix.sum(axis=1)[:, np.newaxis]
    
    # Use a better colormap that shows low values more clearly
    im = ax.imshow(cm_normalized, cmap='Blues', vmin=0, vmax=1)
    
    # Add labels to the axes
    ax.set_xticks(np.arange(len(labels)))
    ax.set_yticks(np.arange(len(labels)))
    ax.set_xticklabels(labels, rotation=90, ha='center', fontsize=8)  # Even smaller font
    ...
    # Only show numbers for non-zero cells to reduce clutter
    for i in range(len(labels)):
        for j in range(len(labels)):
            if confusion_matrix[i, j] > 0:  # Only show non-zero values
                text_color = 'white' if cm_normalized[i, j] > 0.5 else 'black'
                ax.text(j, i, str(int(confusion_matrix[i, j])), ha='center', va='center', 
                       color=text_color, fontsize=6, weight='bold')  # Smaller, bold font
    ...
    ax.set_title('Confusion Matrix', fontsize=16, pad=30)
...
#     plt.rcParams['font.family'] = 'Noto Sans CJK JP'  # change to japanese font
