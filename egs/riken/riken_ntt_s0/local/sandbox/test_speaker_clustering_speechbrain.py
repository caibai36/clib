"""
Family Speaker Clustering using SpeechBrain Speaker Embeddings

This script performs speaker clustering on family recordings (infant, mother, father)
using pre-trained speaker embeddings from SpeechBrain. It processes Audacity
label files and outputs speaker assignments in the same format.

Usage:
    python speechbrain_speaker_clustering.py --audio audio.wav --labels labels.txt
    python speechbrain_speaker_clustering.py --audio audio.wav --labels labels.txt --min-duration 1.0
    python speechbrain_speaker_clustering.py --audio audio.wav --labels labels.txt --num-speakers 2
"""
...
class FamilySpeakerClusteringSpeechBrain:
    """
    Speaker clustering system for family recordings using SpeechBrain embeddings.

    This class performs speaker diarization on family recordings by:
    1. Loading utterance segments from Audacity label files
    2. Extracting speaker embeddings using SpeechBrain ECAPA-TDNN model
    ...
    Attributes:
        encoder: SpeechBrain encoder classifier for speaker embedding extraction
        min_duration: Minimum duration for audio segments (shorter ones get extended)
        device: Computing device (CPU/GPU)
    """

    def __init__(self, min_duration=0.5, device=None):
        """
        ...
        # Load pre-trained SpeechBrain ECAPA-TDNN model
        ...
        try:
            ...
            print("Successfully loaded speechbrain/spkrec-ecapa-voxceleb model")
            ...
            try:
                ...
                print("Successfully loaded speechbrain/spkrec-xvect-voxceleb model")
            except Exception as e2:
                raise Exception(f"Failed to load any SpeechBrain model: {e2}")

        # Set minimum duration for segment extension
        self.min_duration = min_duration
        print(f"Using minimum segment duration: {min_duration}s")

        # Cache for audio duration
    ...
    def get_audio_duration(self, audio_file):
        ...
        if self.audio_duration is None:
            ...
            self.audio_duration = info.num_frames / info.sample_rate
    ...
    def extract_embeddings(self, audio_file, utterances):
        """
        Extract speaker embeddings for each utterance using SpeechBrain.
        ...
        for i, (start, end, content) in enumerate(utterances):
            ...
            try:
                ...
                # Load the extended audio segment using torchaudio
                waveform, sample_rate = torchaudio.load(
                    audio_file,
                    frame_offset=int(extended_start * 16000),  # Assume 16kHz target
                    num_frames=int(extended_duration * 16000)
                )
                ...
                if waveform.shape[1] < 1600:  # Less than 0.1 second at 16kHz
                    print(f"Warning: Very short audio segment ({waveform.shape[1]} samples) for {start}-{end}")

                # Extract embedding using SpeechBrain encoder
    ...
    def process_audio(self, audio_file, label_file, output_file=None, num_speakers=None, max_speakers=3):
        """
        ...
        if output_file is None:
            ...
            output_file = os.path.join(output_dir, f"{base_name}_speakers_{actual_num_speakers}_speechbrain.txt")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Family Speaker Clustering using SpeechBrain Speaker Embeddings",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python speechbrain_speaker_clustering.py -a audio.wav -l labels.txt
  python speechbrain_speaker_clustering.py -a audio.wav -l labels.txt --num-speakers 2
  python speechbrain_speaker_clustering.py -a audio.wav -l labels.txt -m 1.0 -n 3
        """)
    ...
    parser.add_argument('--output', '-o', type=str, default=None,
                       help='Output file path (default: BASENAME_speakers_NUMOFSPEAKERS_speechbrain.txt)')
...
def main():
    ...
    clustering = FamilySpeakerClusteringSpeechBrain(
