...
data_div_yaml="conf/data/division_ntt_riken_model.yaml"  # YAML file containing data division by IDs for train, dev, and test sets
...
window_shift=0.2 # 0.1 # 0.05  # Shift of the sliding window in seconds for label assignment
noise_preserve_steps=20 # 10 # Number of steps to skip between preserved all-noise-no-label chunks # 1 means keeping all noise segments
...
mae_pretrained=""       # "base" for facebook/vit-mae-base, path to pretrained model, or empty string for no pretraining
mae_train_layers=-1  # Train all layers (-1) or specify number of layers to train
...
if [ ${stage} -le 3 ]; then
    date
    echo "Train riken mae vit with linear spectrograms (FFT=512)..."
...
if [ ${stage} -le 4 ]; then
    date
    echo "Test riken mae vit with linear spectrograms (FFT=512) on all test files..."
    python local/sandbox/riken_mae_vit_audio2seg_highres_feat_batch_padding_lin16k_yaml.py \
