...
# Updated to ViT ver1 with a linear warmup and cosine annealing scheduler at 20:23 on 16 August 2024
# Updated to MAE_ViT ver1 with an MAE encoder without masking at 20:42 on December 12 2024.
...
class MAEViTEncoder(nn.Module):
    ...
    def get_attention_maps(self, x):
        ...
        Returns:
            list: Attention weights from each transformer layer
                 Each element has shape (batch_size, num_heads, sequence_length, sequence_length)
        """
        x = self.process_input(x)
        outputs = self.vit(x, output_attentions=True, return_dict=True)
...
def train(model, optimizer, scheduler, train_loader, dev_loader, num_epochs, save_epoch_interval, result_dir, mtest_loader=None):
    ...
    while epoch < num_epochs:
        ...
        # Step the learning rate scheduler and og the current learning rate
        scheduler.step()
...
parser.add_argument("--dim_head", type=int, default=64, help="Dimension of each attention head. Not used in MAE.")
parser.add_argument("--dropout", type=float, default=0.1, help="Dropout rate")
parser.add_argument("--emb_dropout", type=float, default=0.1, help="Embedding dropout rate. Not used in MAE.")
...
model = MAEViTEncoder(
    ...
    pool=args.pool,
    channels=args.channels,
    dim_head=args.dim_head, # not used
    dropout=args.dropout,
    emb_dropout=args.emb_dropout, # not used
    return_attention=args.return_attention,
    return_logits=args.return_logits,
...
if args.pretrained_model:
    ...
    # Note: We don't load the scheduler state as we're using a new scheduling strategy
...
train(model, optimizer, scheduler, train_loader, dev_loader, num_epochs, save_epoch_interval, result, mtest_loader)
