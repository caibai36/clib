...
# Modified to use linear spectrograms at 16kHz with nfft=512 on 09 May 2025
import os
import sys
import random
import re
...
def process_audio_segment(audio_file, segment_file, label2id, middle_part=0.15, window_size=0.5, window_shift=0.15, noise_preserve_steps=5):
    ...
    Creates a linear spectrogram of the audio and generates target labels.
    ...
    Extract 257x256 linear spectrogram for each chunk as input.
    ...
    Returns:
        tuple: A tuple containing the input linear spectrogram and target labels.
            - input_spec (np.ndarray): Input linear spectrogram of shape (num_chunks, 257, 256).
    ...
    # Using 512 for nfft (modified from 1024)
    nfft = 512  # Smaller FFT size for linear spectrogram
    ...
    for i in range(first, length):
        # Create the linear spectrogram if the current piece is not labeled as noise
        ...
        if target[i] != label2id['noise'] or i % noise_preserve_steps == 0:
            ...
            # Got a linear spectrogram image of size 257x256 (freq, time) for a 500ms segment
            ...
            # shift = (500/1000*16000 - 512) / (256-1) = ~30.2
            # Frame size = 512/16000*1000 = 32ms
            # Frame shift = 30.2/16000*1000 = 1.89ms
            # Frame size 32ms and shift 1.89ms with 94% ((32-1.89)/32) overlap
            hop_length = int((window_size*sample_rate - nfft) // (256 - 1)) # Calculate hop_length for 256 frames

            # Initialize the Spectrogram transform for linear spectrogram
...
# # For linear spectrogram with nfft=512
# def create_spec_data(wav_file, segment_duration=2.5, sample_rate=16000, nfft=512, win_size=0.5):
#     """
#     Generates linear spectrogram data from an input WAV file and saves it to a specified location.
#
#     This function reads an audio file, segments it into chunks of specified duration,
#     resamples to 16kHz if necessary, computes the linear spectrogram for each segment, and then saves
#     the collection of linear spectrograms to a file.
#     The function returns the linear spectrogram data as a NumPy array in 16-bit float format
#
#     Args:
...
#         # Using nfft=512 gives us 257 frequency bins (512/2 + 1 = 257)
...
parser.add_argument("--out_dir", type=str, default="exp/data/division_sample_winmid0.05size0.5shift0.05_noisekeep5_linear512", help="Output directory to store the spectra and labels for datasets.")
...
# Modified section: Process each dataset separately to reduce memory usage
...
np.save(os.path.join(save_dir, 'train_input.npy'), train_input)
np.save(os.path.join(save_dir, 'train_target.npy'), train_target)
logger.info(f"train_input.npy with the shape of {train_input.shape}")
logger.info(f"train_target.npy with the shape of {train_target.shape}")
...
np.save(os.path.join(save_dir, 'dev_input.npy'), dev_input)
np.save(os.path.join(save_dir, 'dev_target.npy'), dev_target)
logger.info(f"dev_input.npy with the shape of {dev_input.shape}")
logger.info(f"dev_target.npy with the shape of {dev_target.shape}")
del dev_input, dev_target
gc.collect()
...
np.save(os.path.join(save_dir, 'test_input.npy'), test_input)
np.save(os.path.join(save_dir, 'test_target.npy'), test_target)
logger.info(f"test_input.npy with the shape of {test_input.shape}")
logger.info(f"test_target.npy with the shape of {test_target.shape}")
...
# # Print out the shapes - removed since we already logged them above
# for uttid in data['test']:
