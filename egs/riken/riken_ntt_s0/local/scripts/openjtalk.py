...
with readfile(args.input) as f:
    for line in f:
        ...
        if (args.has_uttid):
             if (len(re.split(r'\s+', line)) <= 1): # empty line or uttid with empty content
                 uttid = line
                 line = ""
             else:
                 uttid, line = re.split(r'\s+', line, maxsplit=1)
             print(uttid, end=" ")

#        for token in pyopenjtalk.run_frontend(line)[0]:
#            print(pyopenjtalk.run_frontend(line))
        for item in pyopenjtalk.run_frontend(line):
            # Convert the dictionary format to the old comma-separated string format
            if isinstance(item, dict):  # Check if we're dealing with the new format
                # Recreate the comma-separated string format from dictionary
                token = f"{item['string']},{item['pos']},{item['pos_group1'] or '*'},{item['pos_group2'] or '*'},{item['pos_group3'] or '*'},{item['ctype'] or '*'},{item['cform'] or '*'},{item['orig']},{item['read']},{item['pron']}"

                # Add accent, mora_size, chain_rule, chain_flag if they exist
                if 'acc' in item and 'mora_size' in item:
                    token += f",{item['acc']}/{item['mora_size']}"
                if 'chain_rule' in item:
                    token += f",{item['chain_rule'] or '*'}"
                if 'chain_flag' in item:
                    token += f",{item['chain_flag']}"
            else:
