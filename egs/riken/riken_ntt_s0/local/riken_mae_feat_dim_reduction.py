...
def main():
    ...
    # Check if age_days column exists
    if 'age_days' not in config_df.columns:
        print("WARNING: 'age_days' column not found in config file. Skipping age filtering.")
        print("Available columns:", list(config_df.columns))
        # No filtering - use all data
        filtered_config_df = config_df.copy()
        filtered_features = features.copy()
        # Create a mask of all True values for consistency
        age_mask = pd.Series([True] * len(config_df))
    else:
    ...
    if nan_count > 0:
        ...
        config_df = config_df[nan_mask]

        print(f"After NaN filtering:")
        print(f"  - Features shape: {features.shape}")
        print(f"  - Config rows: {len(config_df)}")
        print(f"  - Kept {len(config_df) / len(config_df.reset_index()) * 100:.2f}% of samples")
    else:
