...
parser.add_argument('--label_downsampling', type=int, default=1, help="take the labels from a label sequence every n (label_downsampling) steps (label_seq[::n]) (default 1).")
...
for dset in {'train', 'dev', 'test'}:
    ...
    dataset = KaldiDataset(instances, field_to_sort='num_frames', label_downsampling=opts['label_downsampling']) # Every batch has instances with similar lengths, thus less padded elements; required by pad_packed_sequence (pytorch < 1.3)
