...
def process_audio_segment(audio_file, segment_file, label2id, middle_part=0.15, window_size=0.5, window_shift=0.15, noise_preserve_steps=5):
    ...
    for i in range(first, length):
        ...
        if target[i] != label2id['noise'] or i % noise_preserve_steps == 0:
            ...
            hop_length = int((window_size*48000 - nfft) // (256 - 1)) # Calculate hop_length for 256 frames, hop_length 92
            ...
            result = spec_db[0].detach().cpu().numpy() # The first dimension is the batch size
...
def create_spec_data(wav_file, segment_duration=2.5, sample_rate=48000, nfft=512, win_size=0.5):
    """
