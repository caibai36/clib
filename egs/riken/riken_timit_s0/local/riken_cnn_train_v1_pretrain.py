...
parser.add_argument('--pretrained_model', default="", help="the path to pretrained model (model.ckpt). Example usage: python local/riken_cnn_train_v1_pretrain.py --pretrained_model conf/model/model.ckpt --label2id_yaml conf/dict/label2id.yaml")
...
if pretrained_model:
    model.load_state_dict(torch.load(pretrained_model, map_location=device)["model"])
    optimizer.load_state_dict(torch.load(pretrained_model, map_location=device)["optimizer"])
