...
def create_spec_data_batch(wav_file, segment_duration=2.5, sample_rate=16000, nfft=512, win_size=0.5, keep_last_incomplete_segment=False, use_padding=False, feat_batch_size=32):
    """
    Generates linear spectrogram data from an input WAV file with optional padding, using batch processing.
...
class MAEViTEncoder(nn.Module):
    """
    Vision Transformer (ViT) encoder using MAE's architecture without masking mechanism for single-channel linear spectrogram input.

    This model:
    - Uses the encoder part of MAE architecture without any masking
    - Handles single-channel to 3-channel conversion for linear spectrograms
    ...
    def process_input(self, x):
        """
        Process single-channel linear spectrogram input to match MAE's expected input format.
...
def extract_spectrogram_segments_with_padding_batch(audio_file, window_size=0.5, window_shift=0.15, nfft=512, sample_rate=16000, feat_batch_size=32, use_padding=False):
    """
    Extract linear spectrogram segments from an audio file using a sliding window, processing in batches.
...
# Update default paths for MAE ViT model with linear 16k support
default_model = "exp/sys_mae_vit/timit/division_timit_winmid0.05size0.5shift0.05_noisekeep1_lin16k/mae_vit-run0/bs256baselr0.0001wd0.3warmup5total25avgpredwin5_dim768depth12heads12mlpdim3072patch16dropout0.1maetrain6maepretrainedbase/train/model.ckpt"
default_wav_file = "/data/share/bin-wu/data/human/speech/timit/processsing/timit/wav/FELC0_SI1386.wav"
default_dict = "conf/dict/timit_label2id.yaml"
default_resolution=0.01

parser = argparse.ArgumentParser(description="Convert from a audio to its segment file using MAE ViT model with 16kHz linear spectrograms with 512-point FFT. Note that end segments might be discarded in prediction when creating spectra input or averaging the predicted windows. Consider the --padding option to enable prediction at the beginning and end of the audio file.")
...
# Data arguments
parser.add_argument("--batch_size", type=int, default=25, help="Batch size for the dataloader (better not change it)")
parser.add_argument("--feat_batch_size", type=int, default=20480, help="Feat batch size for the extracting features")
...
parser.add_argument("--pred_resolution", type=float, default=default_resolution, help="The resolution of the prediction. The final segments are merged from predicted labels of x-seconds middle parts of a sliding window with the window shift x-seconds. The resolution would be safe when less than 0.05 seconds. Default 0.01. Need to adjust fixed_factor such as set it 0 to match pred_resolution")
...
parser.add_argument("--fixed_factor", type=float, default=0,
                    help="Fixed factor added to segment times. Default -1 (auto-calculate). Use -1 for marmoset data. (default is 0)")
parser.add_argument("--padding", action="store_true", help="(deprecated, always add padding) Add padding to the audio to enable prediction at the start and end")
...
if args.fast_pred:
    merged_seg_file = args.out_seg_file or os.path.join(out_dir, f"mae_vit_pred_{file_name}_lin16k.txt") # When out_file not specified, use out_dir
else:
    merged_seg_file = args.out_seg_file or os.path.join(out_dir, f"mae_vit_pred_resol{args.pred_resolution}_{file_name}_lin16k.txt") # When out_file not specified, use out_dir
