...
def set_device(gpu):
    ...
    return device
def create_spec_data(wav_file, segment_duration=2.5, sample_rate=16000, nfft=1024, win_size=0.5, keep_last_incomplete_segment=False, use_padding=False):
    """
    Generates mel spectrogram data from an input WAV file with optional padding.
...
def extract_spectrogram_segments(audio_file, window_size=0.5, window_shift=0.15, nfft=1024, sample_rate=16000):
    """
    Extract mel spectrogram segments from an audio file using a sliding window.
    ...
    Args:
        ...
        nfft (int): The number of FFT points (default: 1024).
...
def extract_spectrogram_segments_with_padding(audio_file, window_size=0.5, window_shift=0.15, nfft=1024, sample_rate=16000, use_padding=False):
    """
    Extract mel spectrogram segments from an audio file using a sliding window with optional padding.
...
default_model = "exp/sys/timit/division_timit_winmid0.05size0.5shift0.05_noisekeep1_mel16k/cnn-run0/bs2048lr0.0003lrdecay1avgpredwin5/train/model.ckpt"
default_wav_file = "/data/share/bin-wu/data/human/speech/timit/processsing/timit/wav/FELC0_SI1386.wav"
# default_model = "conf/model/b0family3010_best_dev_high_res_0.01sec.ckpt"
...
parser.add_argument("--label2id_yaml", type=str, default="conf/dict/timit_label2id.yaml", help="The YAML file that contains the label-to-labelID mapping.")
...
parser.add_argument("--padding", action="store_true", help="(Deprecated, always use padding) Add padding to the audio to enable prediction at the start and end")
...
if args.fast_pred:
    merged_seg_file = args.out_seg_file or os.path.join(out_dir, f"cnn_pred_{file_name}_mel16k.txt") # When out_file not specified, use out_dir
else:
    merged_seg_file = args.out_seg_file or os.path.join(out_dir, f"cnn_pred_resol{args.pred_resolution}_{file_name}_mel16k.txt") # When out_file not specified, use out_dir
