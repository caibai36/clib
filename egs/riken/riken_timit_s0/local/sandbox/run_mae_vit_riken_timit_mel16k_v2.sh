...
if [ ${stage} -le 3 ]; then
    date
    echo "Train riken mae vit with 16kHz mel spectrograms..."
...
if [ ${stage} -le 4 ]; then
    date
    echo "Test riken mae vit on all test files with 16kHz mel spectrograms..."
    python local/sandbox/riken_mae_vit_audio2seg_highres_padding_mel16k_yaml.py \
...
if [ ${stage} -le 5 ]; then
    date
    echo "Evaluate segments from riken mae vit with 16kHz mel spectrograms..."
