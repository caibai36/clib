...
def create_spec_data_batch(wav_file, segment_duration=2.5, sample_rate=48000, nfft=512, win_size=0.5, keep_last_incomplete_segment=False, use_padding=False, feat_batch_size=32):
    """
    Generates spectrogram data from an input WAV file with optional padding, using batch processing.
    ...
    Returns:
        np.ndarray: Spectrogram data as a NumPy array in 16-bit float format
...
def extract_spectrogram_segments_with_padding_batch(audio_file, window_size=0.5, window_shift=0.15, nfft=512, sample_rate=48000, feat_batch_size=32, use_padding=False):
    """
...
# Update default paths for MAE ViT model
default_model = "exp/sys_mae_vit/timit/division_timit_winmid0.05size0.5shift0.05_noisekeep1/mae_vit-run0/bs256baselr0.0001wd0.3warmup5total25avgpredwin5_dim768depth12heads12mlpdim3072patch16dropout0.1maetrain6maepretrainedbase/train/model.ckpt"
...
if args.fast_pred:
    merged_seg_file = args.out_seg_file or os.path.join(out_dir, f"mae_vit_pred_{file_name}.txt") # When out_file not specified, use out_dir
else:
    merged_seg_file = args.out_seg_file or os.path.join(out_dir, f"mae_vit_pred_resol{args.pred_resolution}_{file_name}.txt") # When out_file not specified, use out_dir
