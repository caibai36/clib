...
if [ ${stage} -le 2 ]; then
    date
    echo "Preparing spectra and labels for datasets using mel spectrograms (FFT=1024)..."
...
if [ ${stage} -le 3 ]; then
    date
    echo "Train riken cnn with mel spectrograms (FFT=1024)..."
...
if [ ${stage} -le 4 ]; then
    date
    echo "Test riken cnn with mel spectrograms (FFT=1024) on all test files..."
    python local/sandbox/riken_cnn_audio2seg_highres_padding_mel16k_yaml.py \
