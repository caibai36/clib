...
"""
Feature Extraction and Dataset Preparation Script for Riken Infant Data
...
2. Split data into train/dev/test sets based on age_months
...
def find_audio_path(root_path, audioid):
    ...
    Args:
        ...
        audioid (str): Audio ID to search for (e.g., "HR01_001_1")
    ...
    Example:
        >>> find_audio_path("/data/riken_infant/", "HR01_001_1")
        "/data/riken_infant/HR01/HR01_001_1.wav"
    """
    matching_files = []
    for dirpath, dirnames, filenames in os.walk(root_path):
        for filename in filenames:
            # Look for exact audioid match in filename
    ...
    for i in range(0, len(audio_paths), batch_size):
        ...
        for path, begin, end in zip(batch_paths, batch_begins, batch_ends):
            ...
            if path in audio_cache:
                ...
                data = data.to(device)  # Move to device first
        ...
        # Stack all signals in batch (they're already on the correct device)
...
def main():
    ...
    3. Splitting the dataset by age_months
    4. Processing each split
    5. Saving features, labels, and configuration files
    """
    parser = argparse.ArgumentParser(description="""
    Extract linear spectrogram features and prepare train/dev/test sets from CSV file for Riken infant data.
    ...
    parser.add_argument("--csv_path", type=str,
                        default="data/riken_infant/riken_infant_cnn.csv",
                        help="Path to input CSV file")

    parser.add_argument("--audio_root", type=str,
                        default="/data01/share/bin-wu/data/human/speech/riken_infant/riken_infant_miki_renamed",
    ...
    logger.info(f"Found audio files for {len(audio_path_mapping)} out of {len(unique_audioids)} unique audioids")
    if missing_count > 0:
        logger.warning(f"{missing_count} audio files could not be found")

    # Get unique months and sort them
    unique_months = sorted(df.age_months.unique())
    logger.info(f"Found data for months: {unique_months}")
    ...
    logger.info(f"Development months: {dev_months}")
    ...
    logger.info(f"Development samples: {len(dev_df)}")
    ...
    logger.info(f"Feature shape: (257, 256) linear spectrograms")
