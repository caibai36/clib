...
model="exp/sys/infant_miki/division_infant_miki_examples_winmid0.05size0.5shift0.05_noisekeep5/cnn-run0/bs2048lr0.0003lrdecay1avgpredwin5/train/model_best_dev.ckpt"
...
out_dir="riken_infant"  # actual out_dir: "exp/out/${arch}_${model_base}/$out_dir/$file_dir/${file_base}_model_${arch}_${model_base}.txt"
data_dir="/data01/share/bin-wu/data/human/speech/riken_infant/riken_infant_miki_renamed"
...
for file in $(cd "$data_dir"; find . -type f \( -name "*.wav" -o -name "*.WAV" \) | sort); do
    # Get the filename without path
    filename=$(basename "$file")
    # Remove extension (case-insensitive)
    if [[ "$filename" == *.WAV ]]; then
        file_base="${filename%.WAV}"
    else
        file_base="${filename%.wav}"
    fi
    ...
    python local/sandbox/riken_cnn_audio2seg_feat_batch.py \
	--label2id_yaml conf/dict/label2id_infant_miki_examples.yaml \
