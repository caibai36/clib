...
3. Filters out samples with NaN values (NEW - based on riken_mae_feat_dim_reduction.py)
4. Performs dimensionality reduction using PCA + t-SNE
5. Saves t-SNE results as separate file
6. Appends t-SNE coordinates to info CSV
...
Notice the difference from riken_mae_feat_dim_reduction.py (it has no random_state)
# tested: results seem no difference
...
def filter_nan_samples(info_df, features):
    """
    Filter out samples containing NaN values in features.
    Based on riken_mae_feat_dim_reduction.py
    ...
    Returns:
        tuple: (filtered_info_df, filtered_features, nan_mask)
    """
    print("\n=== NaN FILTERING ===")
    print(f"Checking for NaN values in features...")

    # Create mask for rows without any NaN
    nan_mask = ~np.isnan(features).any(axis=1)  # True for rows without any NaN
    nan_count = np.isnan(features).sum()
    samples_with_nan = (~nan_mask).sum()

    if nan_count > 0:
        print(f"WARNING: Found {nan_count} NaN values in {samples_with_nan} samples.")
        print(f"Filtering out {samples_with_nan} samples containing NaN values.")

        # Apply NaN filtering mask
        filtered_features = features[nan_mask]
        filtered_info_df = info_df[nan_mask].copy()

        print(f"After NaN filtering:")
        print(f"  - Features shape: {filtered_features.shape}")
        print(f"  - Info rows: {len(filtered_info_df)}")
        print(f"  - Kept {len(filtered_info_df) / len(info_df) * 100:.2f}% of samples")
    else:
        print(f"No NaN values found in features")
        filtered_features = features
        filtered_info_df = info_df.copy()

    return filtered_info_df, filtered_features, nan_mask
...
def process_experiment(info_csv, mae_features_path, output_dir,
    ...
    # NEW: Apply NaN filtering (based on riken_mae_feat_dim_reduction.py)
    info_df, mae_features, nan_mask = filter_nan_samples(info_df, mae_features)
    ...
    # Save NaN mask
    nan_mask_file = os.path.join(output_dir, 'nan_mask.txt')
    np.savetxt(nan_mask_file, nan_mask.astype(int), fmt='%d')
    print(f"Saved NaN mask to {nan_mask_file}")
    ...
    if age_mask is not None:
        print(f"  - {mask_file}")
    print(f"  - {nan_mask_file}")
    print(f"Log saved to: {log_file}")

def main():
    parser = argparse.ArgumentParser(
        description='Perform dimensionality reduction on MAE features with optional age filtering and NaN removal',
