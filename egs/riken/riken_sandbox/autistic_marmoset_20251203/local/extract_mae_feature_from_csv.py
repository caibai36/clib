...
def process_csv(csv_path, wav_scp_path, output_dir, mae_model_path,
    ...
    logger.info(f"Loaded {len(audio_path_mapping)} audio paths from {wav_scp_path}")
...
def main():
    ...
    parser.add_argument('--wav_scp', type=str, required=True,
                        help='Path to wav.scp file (e.g., data/local/Experiment_1/wav.scp)')
