...
dpgmm_train=exp/dpgmm2asr/asr_dpgmm_seed${seed}_K${K0}/data/train/feats.scp
dpgmm_dev=exp/dpgmm2asr/asr_dpgmm_seed${seed}_K${K0}/data/dev/feats.scp
dpgmm_test=exp/dpgmm2asr/asr_dpgmm_seed${seed}_K${K0}/data/test/feats.scp

root=exp/hybrid2asr_mse/asr_hybrid_mse_seed${seed}nseed${nseed}_K${K0}_l${num_left_context}r${num_right_context}_bs${hybrid_batch_size}_hd${hybrid_hidden_dim}_nl${hybrid_num_layers}_ne${hybrid_num_epochs}
data_dir=$root/data
if [ $stage -le 1 ]; then
...
if [ ${stage} -le -1 ]; then # -1 when dict not needed
...
feat=hybrid_mse_seed${seed}nseed${nseed}_K${K0}_l${num_left_context}r${num_right_context}_bs${hybrid_batch_size}_hd${hybrid_hidden_dim}_nl${hybrid_num_layers}_ne${hybrid_num_epochs}
if [ $stage -le 5 ]; then
    mkdir -p $root/conf
