...
root=exp/hybrid2asr/asr_hybrid_seed${seed}_K${K0}_l${num_left_context}r${num_right_context}
data_dir=$root/data
if [ $stage -le 1 ]; then
...
#    rm -rf $data_dir
    ...
    date
    python local/hybrid2asrdata.py \
...
feat=hybrid_seed${seed}_K${K0}_l${num_left_context}r${num_right_context}
if [ $stage -le 5 ]; then
    mkdir -p $root/conf
