...
if [ ${feat_name} = mfcc39 ]; then
    feat_train=data/train/feats.scp
    feat_dev=data/dev/feats.scp
    feat_test=data/test/feats.scp
fi

feat_train2=exp/dpgmm2asr/asr_dpgmm_seed${seed}_K${K0}/data/train/feats.scp
feat_dev2=exp/dpgmm2asr/asr_dpgmm_seed${seed}_K${K0}/data/dev/feats.scp
feat_test2=exp/dpgmm2asr/asr_dpgmm_seed${seed}_K${K0}/data/test/feats.scp

root=exp/concatfeat2asr/asr_${feat_name}_dpgmm_embedding_seed${seed}_K${K0}
data_dir=$root/data
if [ $stage -le 1 ]; then
...
feat=concat_${feat_name}_dpgmm_embedding_seed${seed}_K${K0}
if [ $stage -le 5 ]; then
    mkdir -p $root/conf
