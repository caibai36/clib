...
seed=123
K0=39 # at most K0 + 1 dimension, additional 1 for possible new clusters
...
root=exp/dpgmm2asr/asr_dpgmm_seed${seed}_K${K0}
data_dir=$root/data
if [ $stage -le 1 ]; then
...
#    cp -r data/test $root/data/test
    ...
    python local/dpgmm2asrdata.py \
	   ...
	   --seed=$seed \
	   --alpha=$alpha \
	   --lmbda=$lmbda |& tee exp/logs/run_$(basename ${root}).log
    date
...
# if [ $stage -le 5 ]; then
#     mkdir -p $root/conf
#     if [ -f $root/data/train/utts.json ]; then echo train: \'${PWD}/${root}/data/train/utts.json\'; fi > $root/conf/data_dpgmm_seed${seed}_K${K0}.yaml
#     if [ -f $root/data/dev/utts.json ]; then echo dev: \'${PWD}/${root}/data/dev/utts.json\'; fi >> $root/conf/data_dpgmm_seed${seed}_K${K0}.yaml
#     if [ -f $root/data/test/utts.json ]; then echo test: \'${PWD}/${root}/data/test/utts.json\'; fi >> $root/conf/data_dpgmm_seed${seed}_K${K0}.yaml
#     echo "token2id: '/project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/timit/zr0/data/lang_1char/train_units.txt'" >> $root/conf/data_dpgmm_seed${seed}_K${K0}.yaml
# fi

# feat=dpgmm_seed${seed}_K${K0}
# if [ ${stage} -le 6 ]; then
#     echo "Training ASR..."
