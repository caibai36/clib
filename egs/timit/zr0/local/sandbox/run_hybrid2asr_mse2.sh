...
root=exp/hybrid2asr_mse/asr_hybrid_mse_seed${seed}nseed${nseed}_K${K0}_l${num_left_context}r${num_right_context}
data_dir=$root/data
if [ $stage -le 1 ]; then
...
#    rm -rf $data_dir
    ...
    date
    python local/hybrid2asrdata_mse.py \
...
feat=hybrid_mse_seed${seed}nseed${nseed}_K${K0}_l${num_left_context}r${num_right_context}
if [ $stage -le 5 ]; then
    mkdir -p $root/conf
