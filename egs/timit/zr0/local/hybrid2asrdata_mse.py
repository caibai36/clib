...
class ParallelMSENet(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_layers: int) -> None:
        """
        A neural network with mse loss for parallel dataset with contexts.
    ...
    def forward(self,
        ...
        target: shape (seq_length, output_dim)
        """
...
parser.add_argument("--result", type=str, default="exp/hybrid2asr_mse/test3utt/data",
...
model = ParallelMSENet(input_dim, hidden_dim, output_dim, num_layers)
model.to(device)
print(model)
