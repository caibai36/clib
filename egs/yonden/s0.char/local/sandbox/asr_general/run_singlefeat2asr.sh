...
train_data=$PWD/data/train # /project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/wsj/s0/data/train_si84_first1000
...
bnf_feat=false # whether the feature is bottleneck feature
bnf_dir=/project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/wsj/bnf42/param_bnf # the dir (e.g. param_bnf) containing bnf ark files (e.g. param_bnf/raw_bnfeat_train_si284.1.ark) 

feat_train=/project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/wsj/bnf42/data_bnf/train_bnf/feats.scp
feat_dev=/project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/wsj/bnf42/data_bnf/dev93_bnf/feats.scp
feat_test=/project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/wsj/bnf42/data_bnf/eval92_bnf/feats.scp
...
# special operations for bnf features.
if $bnf_feat; then
    file=param_bnf; if [[ -L "$file" ]]; then rm -rf $file; fi # $file is a symlink to a directory; safe to remove
    ln -s $bnf_dir param_bnf # a feature stored as param_bnf/raw_bnfeat_train_si284.1.ark
fi
if $bnf_feat;then dir=bnf2asr; else dir=singlefeat2asr; fi

root=exp/${dir}/asr_${data_name}_${tag}_single_${feat_name}
feat=${data_name}_${tag}_single_${feat_name}
...
if [ ${stage} -le 4 ]; then
	...
	x=${dataset}
	if [ ! -z $token_scp_name ]; then
	    local/scripts/data2json.sh --feat ${root}/data/$x/feats.scp \
				       --token_scp ${root}/data/$x/${token_scp_name} \
    				       --non-ling-syms ${non_ling_syms} \
