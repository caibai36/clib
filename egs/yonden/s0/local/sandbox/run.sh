...
if [ ${stage} -le 1 ]; then
    ...
    local/yonden_data_prep_all.sh --stage 2 --corpus $yonden --data_index "{003..028}*" --segmented_recordings $segmented_recordings --dir data/local/all_3to28 
fi

if [ ${stage} -le 1 ]; then
    echo "Data division..."
    # Start from the stage 2, skipping the stage 1 of recovering info.json to original yonden data.
    local/yonden_data_division.sh --stage 2 --info data/local/all_3to28/info.json --dir data/local/division
fi
...
if [ ${stage} -le 4 ]; then
    ...
    for dataset in $train $dev $test; do
	x=${dataset}
	local/yonden_data2json.sh --feat data/$x/feats.scp \
