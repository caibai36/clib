...
exp_dir=exp/tmp_asr_cpc # folder holding results of attentional ASR system
...
if [ $stage == 8 ]; then
    ...
    echo "      ./local/sandbox/asr_general/run_cpc.sh --cat_cpc_asr true --cpc_epoch 3000"
fi
...
if $cat_cpc_asr; then
    vq_train=/project/nakamura-lab08/Work/bin-wu/workspace/projects/transfer/CPC/exp/javanese_mfcc/train_epoch${cpc_epoch}/kaldi/train.scp
    vq_dev=/project/nakamura-lab08/Work/bin-wu/workspace/projects/transfer/CPC/exp/javanese_mfcc/dev_epoch${cpc_epoch}/kaldi/dev.scp
    vq_test=/project/nakamura-lab08/Work/bin-wu/workspace/projects/transfer/CPC/exp/javanese_mfcc/test_epoch${cpc_epoch}/kaldi/test.scp
    feat1_name=${feat_name}
    feat2_name=single_cpc_epoch${cpc_epoch}
    ./local/sandbox/asr_general/run_concatfeat2asr.sh --stage 0 --tag ${tag} --dataset_name ${dataset_name} --data_name ${data_name} \
						    --train_data ${train_data} --dev_data ${dev_data} --test_data ${test_data} --train_set ${train_set} \
						    --feat_train ${train_data}/feats.scp --feat_dev ${dev_data}/feats.scp --feat_test ${test_data}/feats.scp \
