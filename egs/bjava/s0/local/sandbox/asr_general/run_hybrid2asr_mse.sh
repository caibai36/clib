...
# datasets containing acoustic features
train_data=$PWD/data/train # /project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/wsj/s0/data/train_si284
...
dpgmm_train=exp/dpgmm2asr/asr_default_asr_general_dpgmm_seed123_K98_from_MFCC39/data/train/feats.scp # exp/dpgmm2asr/asr_wsj1_si284_dpgmm_seed123_K98/data/train/feats.scp
dpgmm_dev=exp/dpgmm2asr/asr_default_asr_general_dpgmm_seed123_K98_from_MFCC39/data/dev/feats.scp # exp/dpgmm2asr/asr_wsj1_si284_dpgmm_seed123_K98/data/dev/feats.scp
dpgmm_test=exp/dpgmm2asr/asr_default_asr_general_dpgmm_seed123_K98_from_MFCC39/data/test/feats.scp # exp/dpgmm2asr/asr_wsj1_si284_dpgmm_seed123_K98/data/test/feats.scp
...
num_left_context=8 # 0
num_right_context=8 # 0
hybrid_batch_size=256
hybrid_hidden_dim=512
hybrid_num_layers=5 # 3
hybrid_num_epochs=20
print_interval=10000
nseed=123 # seed for neural network

# false when only need feature extraction
run_asr=true
...
root=exp/hybrid2asr_mse/asr_${data_name}_${tag}_hybrid_mse_nseed${nseed}_l${num_left_context}r${num_right_context}_bs${hybrid_batch_size}_hd${hybrid_hidden_dim}_nl${hybrid_num_layers}_ne${hybrid_num_epochs}_from_dpgmm_seed${seed}_K${K0}_with_${feat_name}
feat=${data_name}_${tag}_hybrid_mse_nseed${nseed}_l${num_left_context}r${num_right_context}_bs${hybrid_batch_size}_hd${hybrid_hidden_dim}_nl${hybrid_num_layers}_ne${hybrid_num_epochs}_from_dpgmm_seed${seed}_K${K0}_with_${feat_name}

feat_train=${train_data}/feats.scp
feat_dev=${dev_data}/feats.scp
feat_test=${test_data}/feats.scp
...
if [ $stage -le 1 ]; then
    ...
    cp -r ${train_data} $data_dir/train
    cp -r ${dev_data} $data_dir/dev
    cp -r ${test_data} $data_dir/test
...
if [ $stage -le 5 ]; then
    ...
    echo "token2id: '$PWD/data/lang_1char/${train_set}_units.txt'" >> $root/conf/data_${feat}.yaml
fi
