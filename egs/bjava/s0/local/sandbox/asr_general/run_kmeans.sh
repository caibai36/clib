...
cat_hybrid_bnf_asr=false
cat_vqcpc_asr=false
cat_kmeans_asr=false
...
exp_dir=exp/tmp_asr_vqcpc # folder holding results of attentional ASR system
...
if $cat_hybrid_bnf_asr; then
    root1=exp/concatfeat2asr/asr_default_asr_general_concat_MFCC39_hybrid_mse_nseed3009_l8r8_bs256_hd512_nl5_ne20_from_dpgmm_seed123_K98_with_MFCC39
    train1=$root1/data/train/feats.scp
    dev1=$root1/data/dev/feats.scp
    test1=$root1/data/test/feats.scp

    root2=exp/bnf2asr/asr_default_asr_general_mode49_single_BNF42 # exp/bnf2asr/asr_${data_name}_${tag}_single_BNF${bnf_dim}
    train2=$root2/data/train/feats.scp
    dev2=$root2/data/dev/feats.scp
    test2=$root2/data/test/feats.scp

    feat1_name=mfccrnn
    feat2_name=bnf
    ./local/sandbox/asr_general/run_concatfeat2asr.sh --stage 0 --tag ${tag} --dataset_name ${dataset_name} --data_name ${data_name} \
						    --train_data ${train_data} --dev_data ${dev_data} --test_data ${test_data} --train_set ${train_set} \
						    --feat_train ${train1} --feat_dev ${dev1} --feat_test ${test1} \
						    --feat_train2 ${train2} --feat_dev2 ${dev2} --feat_test2 ${test2} \
						    --feat1_name ${feat1_name} --feat2_name ${feat2_name} \
						    --asr_seed ${asr_seed} --gpu ${gpu} --batch_size ${batch_size} --cutoff ${cutoff} --label_smoothing ${label_smoothing} `# config asr`\
						    --lr ${lr} --num_epochs ${num_epochs} --grad_clip ${grad_clip} --factor ${factor} --patience ${patience} --save_interval ${save_interval} `# train asr`\
						    --set_uttid ${set_uttid} --search ${search} --max_target ${max_target} --beam_size ${beam_size} `# eval asr`\
...
if $cat_vqcpc_asr; then
    vq_train=/project/nakamura-lab08/Work/bin-wu/workspace/projects/transfer/VectorQuantizedCPC/exp/javanese_mfcc/train_epoch${vqcpc_epoch}/kaldi/train.scp
    vq_dev=/project/nakamura-lab08/Work/bin-wu/workspace/projects/transfer/VectorQuantizedCPC/exp/javanese_mfcc/dev_epoch${vqcpc_epoch}/kaldi/dev.scp
    vq_test=/project/nakamura-lab08/Work/bin-wu/workspace/projects/transfer/VectorQuantizedCPC/exp/javanese_mfcc/test_epoch${vqcpc_epoch}/kaldi/test.scp
    feat1_name=${feat_name}
    feat2_name=single_vqcpc_epoch${vqcpc_epoch}
    ./local/sandbox/asr_general/run_concatfeat2asr.sh --stage 0 --tag ${tag} --dataset_name ${dataset_name} --data_name ${data_name} \
						    --train_data ${train_data} --dev_data ${dev_data} --test_data ${test_data} --train_set ${train_set} \
						    --feat_train ${train_data}/feats.scp --feat_dev ${dev_data}/feats.scp --feat_test ${test_data}/feats.scp \
						    --feat_train2 ${vq_train} --feat_dev2 ${vq_dev} --feat_test2 ${vq_test} \
						    --feat1_name ${feat1_name} --feat2_name ${feat2_name} \
						    --asr_seed ${asr_seed} --gpu ${gpu} --batch_size ${batch_size} --cutoff ${cutoff} --label_smoothing ${label_smoothing} `# config asr`\
						    --lr ${lr} --num_epochs ${num_epochs} --grad_clip ${grad_clip} --factor ${factor} --patience ${patience} --save_interval ${save_interval} `# train asr`\
						    --set_uttid ${set_uttid} --search ${search} --max_target ${max_target} --beam_size ${beam_size} `# eval asr`\
...
if $cat_kmeans_asr; then
    km_train=/project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/bjava/s0/feat/kmeans_feat/train.scp
    km_dev=/project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/bjava/s0/feat/kmeans_feat/dev.scp
    km_test=/project/nakamura-lab08/Work/bin-wu/workspace/projects/clib/egs/bjava/s0/feat/kmeans_feat/test.scp
    feat1_name=${feat_name}
    feat2_name=single_kmeans
    ./local/sandbox/asr_general/run_concatfeat2asr.sh --stage 0 --tag ${tag} --dataset_name ${dataset_name} --data_name ${data_name} \
						    --train_data ${train_data} --dev_data ${dev_data} --test_data ${test_data} --train_set ${train_set} \
						    --feat_train ${train_data}/feats.scp --feat_dev ${dev_data}/feats.scp --feat_test ${test_data}/feats.scp \
						    --feat_train2 ${km_train} --feat_dev2 ${km_dev} --feat_test2 ${km_test} \
						    --feat1_name ${feat1_name} --feat2_name ${feat2_name} \
						    --asr_seed ${asr_seed} --gpu ${gpu} --batch_size ${batch_size} --cutoff ${cutoff} --label_smoothing ${label_smoothing} `# config asr`\
						    --lr ${lr} --num_epochs ${num_epochs} --grad_clip ${grad_clip} --factor ${factor} --patience ${patience} --save_interval ${save_interval} `# train asr`\
						    --set_uttid ${set_uttid} --search ${search} --max_target ${max_target} --beam_size ${beam_size} `# eval asr`\
